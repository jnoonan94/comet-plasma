...
const int rate_save_t_cst = 20 ;  // For simplicity, keep it an integer fraction of nb_it_max_cst and xLen_cst
const int len_save_t_cst  = int(nb_it_max_cst/rate_save_t_cst) ;

const int xLen_cst = 1024 ;
...
const int nb_part_node_cst = 32 ; //16384 ;
...
struct simu_param {
  ...
  float dX          = .5 ; // Node spacing, in unit of d_i
  float dX_i        = 1./dX ;
  int   nb_dim      = 2 ;

  int   nbPartNode  = nb_part_node_cst ;  // Number of particles per node.

  float dt          = 0.01 ;  // time step, in unit of 1/omega_ci
