...
/* Ion acoustic Landau damping test parameters. */

//______________________________________________________________________________
// Dimensionality & spatial resolutions.
#define NB_DIM 2
//
const int len_x_cst        = 1000;
const int len_y_cst        = 6;
#if NB_DIM==3
  const int len_z_cst      = 999;
#endif
//
const int mpi_nb_proc_y_cst = 1;
const int mpi_nb_proc_z_cst = 1; // Should never be 0! Even in 2D case.
//
const int nb_part_node_cst = 100000;//32768;
//
...
//                                   |
//                                   v
const int rate_save_t_cst         = 1;  // For simplicity, keep it an integer fraction of nb_it_max_cst and len_x_cst
const int len_save_t_cst          = int(nb_it_max_cst/rate_save_t_cst)+1;
const int rate_save_field_cst     = 1000;
const int rate_save_particles_cst = 5000;
//______________________________________________________________________________
...
#define obstacle_cst    false // Adding an obstacle.
...
// Memory/variable sizes.
  ...
  const unsigned int tank_size_cst    = static_cast<unsigned int>(1.01*nb_part_node_cst*len_x_cst*len_y_cst);
  ...
  const unsigned int pool_size_cst    = static_cast<unsigned int>(2.5*nb_part_node_cst*len_x_cst*len_y_cst*len_z_cst);
...
//______________________________________________________________________________
// Other.
const unsigned int nb_part_add_pla_cst = 10000;
//
const int idx_it_inject_cst = 4000;
const std::string path_inputs_str_cst = "../run_050";
//
const int seed_val_cst = 999; //time(NULL);
...
const int nb_probes_y_cst = 2;
#if NB_DIM==2
  const int nb_probes_tot_cst = nb_probes_x_cst*nb_probes_y_cst;
#elif NB_DIM==3
  const int nb_probes_z_cst = 2;
  const int nb_probes_tot_cst = nb_probes_x_cst*nb_probes_y_cst*nb_probes_z_cst;
#endif

const bool smooth_patch_cst = false;
...
struct simu_param {
  ...
  float dt = .002;  // time step, in unit of 1/omega_ci
  ...
  //
  float E_max_smooth = 40.; // If used, above this value in any electric field component, E and B are locally smoothed.
  ...
  //____________________________________________________________________________
  // Obstacle related.
  float Q           = 7.5e26;//1.25e27;  // s-1
  float nu_i        = 1.6e-7;  // s-1
  float nu_d        = 4.5e-6;  // s-1
  float u0          = 700.;  // m/s
  // Ionosphere:
  ...
  float centre_y    = 4./10.;
    ...
    float centre_z  = 1./2.;
  #endif
  float dip_mom_SI [3] = {0., -6.e10, 0.}; // T.m^3
  float r_obs_SI    = 2.440e6; // m
