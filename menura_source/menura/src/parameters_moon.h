...
const int len_y_cst        = 10 ;
#if NB_DIM==3
  const int len_z_cst      = 81 ;
#endif
const int nb_part_node_cst = 10 ;
//
...
const int rate_save_field_cst     = 10 ;
...
// Memory/variable sizes.
  ...
  const unsigned int pool_size_cst    = static_cast<unsigned int>(1.2*nb_part_node_cst*len_x_cst*len_y_cst) ;
  ...
  const unsigned int pool_size_cst    = static_cast<unsigned int>(1.1*nb_part_node_cst*len_x_cst*len_y_cst*len_z_cst) ;
...
//______________________________________________________________________________
// Other.
const unsigned int nb_part_add_pla_cst = 100 ;
//
const int idx_it_inject_cst = 300 ;
//
...
const bool debug_mode_cst = true ;
...
struct simu_param {
  ...
  float dt = .08 ;  // time step, in unit of 1/omega_ci

  //____________________________________________________________________________
  // Background physical values in SI.
  float n0_no_norm  = 10.5e6 ;
  float B0_no_norm  = 4.302941e-9 ;
  //
  float B0_x        = 0.855229 ; // If Alfven.
  float B0_y        = 0.518250 ;
  float B0_z        = 0. ; // If magnetosonic.
  //
  float beta        = 1. ;
  float Ti_inf      = beta*B0_no_norm*B0_no_norm/(n0_no_norm*k_B*2.*mu0) ;
  float Te_inf      = .25*Ti_inf ;
  ...
  float poly_ind     = 1. ;
  //
  float eta_res_norm = 1.e-1 ;
  float eta_hyp_res  = 1.e-2 ;
  float eta_res_vac  = 1.e1 ;
  float eta_res_obs  = 1.e1;//1.e7*e*n0_no_norm/B0_no_norm;
  //
  float eta_sm       = 0. ;
  //
  ...
  float u0          = 1000. ;  // m/s
  int   Z_pla       = 18 ;
  float centre_x    = 5./8. ; //5./8. ;
  float centre_y    = 1./2. ;
  #if NB_DIM==3
    float centre_z  = 1./2. ;
  #endif
  float dip_mom [3] = {0., 0., 0.};
  float r_obs       = 1.730e6; // m
  ...
  void printout_parameters(){
    ...
    printf("|           Proton temperature : %9.4f eV \n", Ti_inf/(e/k_B)) ;
    printf("|         Electron temperature : %9.4f K \n", Te_inf) ;
    printf("|         Electron temperature : %9.4f eV \n", Te_inf/(e/k_B)) ;
    printf("|\n") ;
