...
/* Test-particle/DF parameters. */

//______________________________________________________________________________
// Type of simulation.
#define solve_EB_cst    false // If false, E and B are not solved self-consistently.
#define DF_cst          true // Use a distribution function instead of particles.
//
...
#define NB_DIM 2
//
const int len_x_cst        = 20;
const int len_y_cst        = 100;
#if NB_DIM==3
  const int len_z_cst      = 100;
#endif
const int len_v_cst        = 41;
//
#if !DF_cst
const int nb_part_node_cst = 1000; //16384;
#else
const int nb_part_node_cst = 1;
#endif
...
const int rate_save_particles_cst = 100;
//______________________________________________________________________________
...
// Memory/variable sizes.
  ...
  const unsigned int pool_size_cst    = static_cast<unsigned int>(2.2*nb_part_node_cst*len_x_cst*len_y_cst);
...
struct simu_param {
  ...
  // float dX = (nb_wave*lambda_wave)/xLen;
  float omega_wave = 1.75;
  float period_wave = 2.*PI/omega_wave;

  float dX = .1;//(nb_wave*lambda_wave)/len_x_cst;//.1; // Node spacing, in unit of d_i
  float dt = 0.001; // time step, in unit of 1/omega_ci

  #if DF_cst
  float dV = 10./len_v_cst;
  #endif
  ...
  float poly_ind     = 1.;
  //
  float eta_res_norm = 0.; //1.e-1; //5.e-4; //.00375; //eta_res*q*n0_no_norm/B0_no_norm;
  float eta_hyp_res  = 1.e-2; //5.e-5; //2.e-4; //2.e-4; //.00375; //eta_res*q*n0_no_norm/B0_no_norm;
  float eta_res_vac  = 1.e1;
  float eta_res_obs  = 3.e1;//1.e7*e*n0_no_norm/B0_no_norm;
  //
  float eta_sm       = 0.;
  //
  ...
  //
  int   Z_pla       = 18;
  float centre_x    = 1./2.; //5./8.; 3./8.
  float centre_y    = 1./2.;
  #if NB_DIM==3
    float centre_z  = 1./2.;
  #endif
  float dip_mom [3] = {0., 0., 10000.};
  float r_obs       = 50.;
  float r_obs_sqr   = std::pow(r_obs/d_i, 2);
  ...
  void printout_parameters(){
    ...
    if (!solve_EB_cst){
      printf("|\n");
      printf("| Test-particle simulation. \n" );
      printf("|              Phase speed (v_A): %9.4f \n", omega_wave/k_wave);
      printf("|                %i iterations: %9.4f wave period \n", nb_it_max_cst, nb_it_max_cst*dt/(2.*PI/omega_wave));
    }
    printf("|\n|\n");
