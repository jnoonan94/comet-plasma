...
#define NB_DIM 3
//
const int len_x_cst        = 80;
const int len_y_cst        = 80;
#if NB_DIM==3
  const int len_z_cst      = 80;
#endif
//
const int mpi_nb_proc_y_cst = 1;
const int mpi_nb_proc_z_cst = 1;
//
const int nb_part_node_cst = 5;
//
...
struct simu_param {
  ...
  float dX = 12.; // Node spacing, in unit of d_i
  float dt = .4;  // time step, in unit of 1/omega_ci
  ...
  float centre_y    = 1./2.;
    ...
    float centre_z  = 1./2.;
  #endif
  float dip_mom_SI [3] = {0., -1.e12, 0.}; // T.m^3
